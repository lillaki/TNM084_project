// ------------------ NOISE ---------------------------- //
    ...
    void main()
    {
      ...
      // Draw the iris, r = 0 in the iris middle 
      if( r < 0.2 ){
        ...
        float f = 0.5*fbm( 20.0*uVu, vec2(10.0));
        color = mix(color, vec3(0.2, 0.5, 0.4), f);

        // Inner color 
        f = 1.0 - smoothstep( 0.05, 0.15, r );
        color = mix( color, vec3( 0.8, 0.6, 0.1), f);

        // Deform the angle to create waves in iris 
        //a += 0.5*fbm(vec2(2.0*a, 0.5*r), per-8.0);

        // White fibers
        // Use smoothstep to add contrast
        f = 0.7*fbm(vec2(10.0*a, 3.5*r), per) + 0.2; // Add 0.2 for brighter white
        color = mix( color, vec3(1.0), f );

        // Black fibers
        // Use smoothstep to add contrast
        f = fbm(vec2(9.0*a, 3.0*r), per-1.0);
        color = mix( color, vec3(0.0), f );

        // Volume effect - outer circle of iris
        f = 1.0 - 0.7*smoothstep( 0.12, 0.19, r );
        color *= f;  // Multiply with f to add contrast

        // Pupil
        f = smoothstep( 0.04, 0.07, r );
        color *= f;

        // Smoothen the iris edges
        f = smoothstep( 0.17, 0.2, r );
        color = mix(color, vec3(1.0), f);

      }

      // Deform the angle to create wave formation in veins 
      a += 0.2*fbm(vec2(2.0*a, 0.5*r), per-8.0);

      // Add veins to the sclera
      float v = 0.7*fbm(vec2(14.0*a, r), vec2(14.0));

      vec3 sclera = mix(vec3( 1.0 ), vec3( 1.0, 0.0, 0.0 ), v);
      color = mix(color, sclera, smoothstep(0.2, 0.5, uVu.y));
